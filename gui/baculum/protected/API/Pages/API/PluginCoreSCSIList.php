...
/**
 * List SCSI tape devices on the storage daemon host.
 *
 * @author Marcin Haba <marcin.haba@bacula.pl>
 * @category API
 * @package Baculum API
 */
class PluginCoreSCSIList extends ConsoleOutputQueryPage {
	...
	/**
	 * Get scsilist command output in raw format.
	 *
	...
	protected function getRawOutput($params = []) {
		...
		if ($ret->exitcode != 0) {
			$this->getModule('logging')->log(
				...
				'Wrong output from RAW .query scsilist: ' . implode(PHP_EOL, $ret->output)
			);
	...
	/**
	 * Get query scsilist command output in JSON format.
	 *
	 * @param array $params command parameters
	 * @return StdClass object with output and exitcode
	 */
