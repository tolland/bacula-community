...
void FDStatus::doUpdate()
{
   if (count == 0) {            // We do only doUpdate()
      count++;
      task *t = new task();
      ...
      res->wrk->queue(t);
      status.statusBar->setText(QString("Trying to connect to FD..."));
      Dmsg1(50, "doUpdate(%p)\n", res);
   }
}

void FDStatus::taskDone(task *t)
{
   int nbjobs=0;
   count--;
   if (!t->status) {
      ...
      if (t->type == TASK_STATUS) {
         ...
         status.labelBandwidth->setText(QString(edit_uint64(res->bwlimit, ed1)));
         /* Clear the table first */
         Freeze(*status.tableRunning);
         Freeze(*status.tableTerminated);
         QStringList headerlistR = (QStringList() << tr("JobId")
                                   << tr("Job")  << tr("Level")
                                    << tr("Files") << tr("Bytes") << tr("Errors") << tr("Current File"));
         ...
         if (res->running_jobs) {
            ...
            foreach_alist(rjob, res->running_jobs) {
               ...
               item.setNumericFld(col++, QString(rjob->CurrentFile));
               nbjobs++;
            }
         } else {
         ...
         QStringList headerlistT = (QStringList() << tr("JobId")
                                    << tr("Job")  << tr("Level")
                                    << tr("Status") << tr("Files") << tr("Bytes") << tr("Errors"));
