...
win_tape_device::win_tape_device()
{
   m_fd = -1;
}
...
int
win_tape_device::d_open(const char *file, int flags, ...)
{
...
ssize_t win_tape_device::d_read(int fd, void *buffer, size_t count)
{
   if (buffer == NULL) {
      errno = EINVAL;
      return -1;
...
ssize_t win_tape_device::d_write(int fd, const void *buffer, size_t count)
{
   if (buffer == NULL) {
      errno = EINVAL;
      return -1;
...
int win_tape_device::d_close(int fd)
{
   if (fd < 3 || fd >= (int)(NUMBER_HANDLE_ENTRIES + 3) ||
      TapeHandleTable[fd - 3].OSHandle == INVALID_HANDLE_VALUE) {
      errno = EBADF;
...
int win_tape_device::d_ioctl(int fd, ioctl_req_t request, char *op)
{
   va_list argp;
   int result;
...
int win_tape_device::tape_op(int fd, struct mtop *mt_com)
{
...
int win_tape_device::tape_get(int fd, struct mtget *mt_get)
{
...
int win_tape_device::tape_pos(int fd, struct mtpos *mt_pos)
{
   DWORD partition;
   DWORD offset;
   DWORD offsetHi;
